...
def get_latest_rollback_file(files,
    ...
    """Given a list of rollback file names, return the latest filename
    based on the name's timestamp.
    """
    if not files:
        return None
...
def get_rollback_file(directory=current_dir,
    ...
    rollback_filename = get_latest_rollback_file(
        files,
...
def list_rollback_files(directory,
                        rollback_filename_template="rollback-info_"):
    """List files in the directory whose name starts with
    rollback-info_. Return a list with the filenames.

    Arguments:
    directory -- the directory to search the files
    """
    rollback_files_pattern = directory + os.sep +\
        rollback_filename_template + "*"
...
def generate_rollback_script(args, dir=None):
    ...
    if not controller:
        return "# Cannot initialize the controller {0}.\
 Rollback will not occour.".format(args.controller)
    ...
    rollback_file = get_rollback_file(
        directory=directory,
        rollback_filename_template=rollback_filename_template
    )

    if not rollback_file:
        return "# Cannot find rollback-info file in {0}.\
